...
async function dynamodbStreamProxy(config) {
	const dynamodb = new DynamoDB({ endpoint: config.dbEndpoint || 'http://localhost:5000' , region: process.env.AWS_REGION || config.dbRegion });
	const emitter = new EventEmitter();
		...
		const server = await createProxy(config.proxyPort || 8000, async (req) => {
			...
			const Keys = req.body.Key || Table.KeySchema.reduce((sum, el) => ({
				...sum,
				[el.AttributeName]: body[el.AttributeName]
			}), {})

			const { Item: OldImage } = await dynamodb.getItem({ TableName, Key: Keys }).promise();
		...
		async (state) => {
			const { Item: NewImage } = await dynamodb.getItem({ TableName: state.TableName, Key: state.dynamodb.Keys }).promise()
